...
import { LanguageSwitcher, useLanguage } from "../i18n.js";
import {
  Activity,
  ShieldAlert,
  Cpu,
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const tabs = [
    ...
    { id: "scientific-boundary", label: "Scientific Boundary", category: "foundations", icon: <ShieldAlert className="h-3.5 w-3.5" /> },
    ...
    { id: "api-explorer", label: "API Explorer", category: "runtime", icon: <Terminal className="h-3.5 w-3.5" /> },

    // Governance & Verification (4)
  ...
  const categories: { id: TabCategory; label: string; count: number }[] = [
    ...
    { id: "foundations", label: "Foundations", count: tabs.filter((t) => t.category === "foundations").length },
    { id: "design", label: "Design & Protocols", count: tabs.filter((t) => t.category === "design").length },
    { id: "runtime", label: "Execution & Runtime", count: tabs.filter((t) => t.category === "runtime").length },
    { id: "governance", label: "Governance & Verification", count: tabs.filter((t) => t.category === "governance").length },
  ];

  // When a group is clicked, filter to that category AND switch to its first tab if not already in category
  const handleSelectCategory = (catId: TabCategory) => {
    setSelectedCategory(catId);
    if (catId !== "all") {
      const currentTabObj = tabs.find((t) => t.id === activeTab);
      if (!currentTabObj || currentTabObj.category !== catId) {
  ...
  // Synchronize category filter when activeTab changes externally (e.g., from overview quick actions, tour, cmd+k)
  useEffect(() => {
    if (selectedCategory !== "all") {
      const currentTabObj = tabs.find((t) => t.id === activeTab);
      if (currentTabObj && currentTabObj.category !== selectedCategory) {
        setSelectedCategory(currentTabObj.category as TabCategory);
      }
    }
  }, [activeTab]);
    ...
    <header className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-30 shadow-2xs transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-3">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded text-[11px] font-mono font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                  {isAz ? "L0 → L8 Arxitekturası" : "L0 → L8 Architecture"}
                </span>
              </div>
              <p className="text-[11px] text-amber-700 dark:text-amber-400 font-medium flex items-center gap-1 mt-0.5">
                <ShieldAlert className="h-3.5 w-3.5 flex-shrink-0" />
                {t("Deterministic Research Scaffold • Operational Specifications • Verification Baseline")}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 self-start lg:self-auto">
            ...
            <button
              ...
              title={isAz ? "Sürətli axtarış (Cmd+K / Ctrl+K)" : "Quick search (Cmd+K / Ctrl+K)"}
            >
            ...
            <button
              ...
              title={isAz ? "12 mərhələli tədqiqat həyat dövrü interaktiv turu" : "Take interactive tour of 12-state research lifecycle"}
            >
              <BookOpen className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
              <span>{t("12-State Tour")}</span>
            </button>
            ...
            <button
              ...
              title={isDarkMode ? (isAz ? "Laboratoriya İşıqlı Rejiminə Keç" : "Switch to Laboratory Light Mode") : (isAz ? "Laboratoriya Qaranlıq Rejiminə Keç" : "Switch to Laboratory Dark Mode")}
            >
              {isDarkMode ? (
        ...
        <div className="mt-3.5 pt-2 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2 overflow-x-auto pb-1 text-[11px]">
          <div className="flex items-center gap-1.5">
            <span className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex items-center gap-1 mr-1">
              <Filter className="h-3 w-3" /> {t("Groups:")}
            </span>
            {categories.map((cat) => {
              ...
              return (
                <button
                  key={cat.id}
                  id={`group-filter-${cat.id}`}
                  onClick={() => handleSelectCategory(cat.id)}
                  title={isAz ? `${t(cat.label)} qrupuna keç` : `Switch to ${cat.label} group`}
                  className={`px-2.5 py-1 rounded-md font-medium whitespace-nowrap transition-colors flex items-center gap-1 ${
                    isSelected
                      ? "bg-slate-800 text-white dark:bg-blue-600 shadow-xs ring-1 ring-slate-700 dark:ring-blue-500"
        ...
        <nav
          ...
          aria-label={t("Scientific Platform Navigation")}
        >
          {visibleTabs.map((tab) => {
