...
const ENDPOINTS: Endpoint[] = [
  {
    method: "GET",
    path: "/api/v1/health",
    descriptionEn: "System health, version, and implementation status baseline.",
    descriptionAz: "Sistem sağlamlığı, versiya və tətbiq statusu bazası.",
  },
  {
    method: "GET",
    path: "/api/v1/measurements",
    descriptionEn: "Returns all 170 registered measurement specifications across Tier I, II, III.",
    descriptionAz: "I, II, III səviyyələr üzrə bütün 170 qeydiyyatdan keçmiş ölçmə spesifikasiyasını qaytarır.",
  },
  {
    method: "GET",
    path: "/api/v1/measurements/C01-01",
    descriptionEn: "Lookup a single measurement specification contract.",
    descriptionAz: "Tək bir ölçmə spesifikasiyası müqaviləsini axtarır.",
  },
  {
    method: "POST",
    path: "/api/v1/studies",
    descriptionEn: "Create a new study specification container.",
    descriptionAz: "Yeni tədqiqat spesifikasiyası konteyneri yaradır.",
  ...
  },
  {
    ...
    descriptionEn: "Fetch a sample aggregated outcome contract for a trial session.",
    descriptionAz: "Sınaq sessiyası üçün nümunəvi aqreqasiya edilmiş nəticə müqaviləsini gətirir.",
  },
  {
    method: "POST",
    path: "/api/v1/research/compile-run",
    descriptionEn: "Compile and execute deterministic trials for given measurement IDs.",
    descriptionAz: "Verilmiş ölçmə identifikatorları üçün deterministik sınaqları tərtib edir və icra edir.",
    defaultBody: JSON.stringify(
  ...
  },
  {
    ...
    descriptionEn: "Retrieve complete Unified Research Governance audit record.",
    descriptionAz: "Tam Vahid Tədqiqat İdarəetmə audit qeydini əldə edir.",
  },
];
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900 mb-1 flex items-center gap-2">
          ...
          {isAz ? "İnteraktiv API Tədqiqatçısı" : "Interactive API Explorer"}
        </h2>
        <p className="text-sm text-slate-600">
          {isAz
            ? "Miqrasiya edilmiş arxa plan REST son nöqtələrini (/api/v1/*) birbaşa real vaxtda icra edin və sınaqdan keçirin."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-2">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider px-2 mb-2">
            {isAz ? "Mövcud Son Nöqtələr" : "Available Endpoints"}
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              >
                ...
                {isAz ? "Sorğu Göndər" : "Send Request"}
              </button>
            </div>

            <p className="text-xs text-slate-600">
              {isAz ? selectedEndpoint.descriptionAz : selectedEndpoint.descriptionEn}
            </p>

            {selectedEndpoint.method === "POST" && (
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700">
                  {isAz ? "Sorğu Gövdəsi (JSON)" : "Request Body (JSON)"}
                </label>
                <textarea
                  rows={5}
                  value={requestBody}
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-700 flex items-center gap-2">
                  ...
                  {response.status && (
                    ...
                    >
                      {isAz ? "Status:" : "Status:"} {response.status}
                    </span>
                  )}
                </span>
                <button
