        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2">
              ...
              {isAz ? "Elmi İdarəetmə və Təkrarlanma Reyestri" : "Scientific Governance & Reproducibility Ledger"}
            </h2>
            <p className="text-sm text-slate-600">
              {isAz
                ? "Yalnız oxunabilən idarəetmə auditi konsolu. Xarici öncədən qeydiyyatdan keçmiş sınaqlar olmadan empirik kliniki etibarlılığı nəticə çıxarmır."
          ...
          >
            ...
            {isAz ? "İdarəetmə Görünüşünü Yenilə" : "Refresh Governance Snapshot"}
          </button>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "İdarəetmə Statusu" : "Governance Status"}
              </span>
              ...
              <p className="text-xs text-slate-500">
                {isAz ? "Boru kəməri validasiyası tamamlandı" : "Pipeline validation completed"}
              </p>
            </div>

            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "Analiz Bütövlük Xalı" : "Analysis Integrity Score"}
              </span>
              <div className="text-lg font-bold text-slate-900 font-mono">
                {snapshot.analysis_integrity?.score ?? 0} / 1.00
              </div>
              <p className="text-xs text-slate-500">
                {isAz ? "Hazırlıq:" : "Readiness:"}{" "}
                <span className="font-semibold text-blue-600">{snapshot.analysis_integrity?.readiness}</span>
              </p>
            </div>

            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "Sübut İddia Qrafı" : "Evidence Claim Graph"}
              </span>
              ...
              <p className="text-xs text-slate-500">
                {isAz
                  ? `${snapshot.evidence?.nodes_count} düyün, ${snapshot.evidence?.edges_count} istiqamətləndirilmiş tel`
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "İddia Vəziyyəti" : "Claim State"}
              </span>
              <div className="text-lg font-bold text-slate-900 font-mono">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "Elmi Qərar" : "Scientific Decision"}
              </span>
              <div className="text-lg font-bold text-slate-900">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-1">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                {isAz ? "Reyestrin Kriptoqrafik Bütövlüyü" : "Ledger Cryptographic Integrity"}
              </span>
              <div className="text-lg font-bold text-slate-900 font-mono">
                {snapshot.ledger?.valid
                  ? isAz ? "ETİBARLI" : "VALID"
                  : isAz ? "QEYRİ-ETİBARLI" : "INVALID"}
              </div>
              <p className="text-xs text-slate-500 font-mono truncate" title={snapshot.ledger?.latest_hash}>
                {isAz ? "Qeydlər:" : "Records:"} {snapshot.ledger?.records} | {isAz ? "Heş:" : "Hash:"}{" "}
                {snapshot.ledger?.latest_hash?.slice(0, 16)}...
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900">
                  {isAz ? "Sübutla Əlaqəli Model Reviziyası" : "Evidence-Linked Model Revision"}
                </h3>
              </div>
              {snapshot.model_revision ? (
                <div className="p-3 bg-purple-50 rounded-lg border border-purple-200 text-xs space-y-1 font-mono">
                  <div className="flex justify-between">
                    <span className="text-purple-600 font-sans">{isAz ? "Reviziya ID:" : "Revision ID:"}</span>
                    <span className="font-bold text-purple-900">{snapshot.model_revision.revision_id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-purple-600 font-sans">{isAz ? "Hədəf Model:" : "Target Model:"}</span>
                    <span className="text-purple-900">{snapshot.model_revision.model_id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-purple-600 font-sans">{isAz ? "Ana Model:" : "Parent Model:"}</span>
                    <span className="text-purple-900">{snapshot.model_revision.parent_model_id || (isAz ? "Yoxdur" : "None")}</span>
                  </div>
                  <div className="pt-1 font-sans text-purple-800">
                    <strong>{isAz ? "Səbəb:" : "Reason:"}</strong> {snapshot.model_revision.reason}
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-500 italic">
                  {isAz ? "Mövcud dövr üçün aktiv model reviziyası yoxdur." : "No model revision active for current cycle."}
                </p>
              )}
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-900">
                  {isAz ? "Bloklama Səbəbləri / İlkin Şərtlər" : "Blocking Reasons / Pre-Conditions"}
                </h3>
              </div>
              {snapshot.blocking_reasons.length === 0 ? (
                <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-200 text-emerald-800 text-xs flex items-center gap-2">
                  ...
                  {isAz
                    ? "Bloklayıcı idarəetmə şərtləri aşkar edilmədi. Öncədən qeydiyyat, çoxluq və təsir-qeyri-müəyyənlik tələbləri ödənildi."
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                ...
                {isAz ? "Tam Kriptoqrafik Audit Qeydi (JSON)" : "Full Cryptographic Audit Record (JSON)"}
              </span>
              <button
                onClick={handleCopy}
              ...
              >
                ...
                {copied ? (isAz ? "Kopyalandı" : "Copied") : (isAz ? "Qeydi Kopyala" : "Copy Record")}
              </button>
            </div>
