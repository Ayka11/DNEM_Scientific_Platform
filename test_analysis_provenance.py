    ...
    try:
        ...
        for i in range(12):
            trials.append({
                "trial_id": f"t{i}",
                "session_id": "S1",
                "measurement_id": "FI-01",
                "stimulus_onset": i,
                "response": "A",
                "response_time": 500 + i,
                "correct": 1 if i < 9 else 0,
                "valid": True,
            })
        ...
        verification = ap.verify("AN-000001")
        ...
        try:
            ...
            assert str(e) == "analysis_id_exists"
        else:
            raise AssertionError("duplicate analysis_id accepted")

    finally:
        # Close every SQLite owner before TemporaryDirectory cleanup.
        if getattr(ap, "persistence", None) is not None:
            ap.persistence.close()

        if getattr(ws, "persistence", None) is not None:
            ws.persistence.close()
